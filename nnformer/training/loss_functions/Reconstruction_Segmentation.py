...
class KLloss(nn.Module):
    ...
    def forward(self, z_mean, z_var):
        f = torch.exp(z_var) + torch.square(z_mean) - 1. -z_var
        loss = torch.mean(f)
        return loss

class Rec_Loss(nn.Module):
    def __init__(self, if_MSE=True, if_KL=True):
        super(Rec_Loss, self).__init__()
        self.if_MSE = if_MSE
        self.if_KL = if_KL
        if self.if_MSE:
            self.opt_Mse = nn.MSELoss()
        if self.if_KL:
            self.opt_KL = KLloss()

    def forward(self, image_output, z_mean, z_var,  image):
        if self.if_MSE:
            loss_MSE = self.opt_Mse(image_output, image)
        else:
            loss_MSE = 0
        if self.if_KL:
            loss_KL = self.opt_KL(z_mean, z_var)
        else:
            loss_KL = 0
        loss = 0.0001 * loss_MSE + 0.1 * loss_KL
        return loss

# t= Seg_Rec_Loss({'batch_dice': True, 'smooth': 1e-5, 'do_bg': False}, {})
# x =0
# y =1
# z =2
# m= 3
