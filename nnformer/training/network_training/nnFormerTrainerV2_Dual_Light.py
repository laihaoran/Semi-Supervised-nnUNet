...
from nnformer.network_architecture.generic_UNet_last_cut import Generic_UNet, Generic_LUNet, SeparableConv3d, Generic_Dual_LUNet
...
class nnFormerTrainerV2_Dual_Light(nnFormerTrainer_last_cut):
    """
    ...
    def __init__(self, plans_file, fold, output_folder=None, dataset_directory=None, batch_dice=True, stage=None,
        ...
        self.loss1 = DC_and_CE_loss({'batch_dice': self.batch_dice, 'smooth': 1e-5, 'do_bg': False}, {})
        self.loss2 = Robust_DC_and_CE_loss({'batch_dice': self.batch_dice, 'smooth': 1e-5, 'do_bg': False}, {})
    ...
    def initialize_network(self, training=True):
        ...
        if training:
            self.network = Generic_Dual_LUNet(self.num_input_channels, self.base_num_features, self.num_classes,
            ...
            self.network = Generic_LUNet(self.num_input_channels, self.base_num_features, self.num_classes,
                                         len(self.net_num_pool_op_kernel_sizes),
                                         self.conv_per_stage, 2, conv_op, norm_op, norm_op_kwargs, dropout_op,
                                         dropout_op_kwargs,
                                         net_nonlin, net_nonlin_kwargs, True, False, lambda x: x, InitWeights_He(1e-2),
    ...
    def run_training(self):
        ...
        while self.epoch < self.max_num_epochs:
            ...
            if self.epoch == 0:
                self.loss = MultipleOutputLoss2(self.loss1, self.ds_loss_weights)
                self.print_to_log_file("Using DC_Dice Loss")
            elif self.epoch == 200:
                self.loss = MultipleOutputLoss2(self.loss2, self.ds_loss_weights)
                self.print_to_log_file("Now loss function is changing, using Robust DC_Dice Loss")
