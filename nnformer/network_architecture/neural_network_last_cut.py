...
class SegmentationNetwork(NeuralNetwork):
    ...
    def short_time_3D_prediction(self, aggregated_results, aggregated_nb_of_predictions):
        stop = True
        class_probabilities = aggregated_results / aggregated_nb_of_predictions
        predicted_segmentation = class_probabilities.argmax(0)
        if np.sum(predicted_segmentation) != 0:
            if np.sum(predicted_segmentation[-1]) == 0:
                return stop
            else:
                return not stop
        else:
    ...
    def inwhole_3D_prediction_process(self, aggregated_results, aggregated_nb_of_predictions, data_shape):
        class_probabilities = aggregated_results / aggregated_nb_of_predictions
        part_seg = class_probabilities.argmax(0)
        predicted_segmentation = np.zeros(list(data_shape[1:]), dtype=np.uint8)
        predicted_segmentation[:part_seg.shape[0]] = part_seg

        # aggregated_results[0:1, 0:z_index,] = 1
        # aggregated_nb_of_predictions[:, 0:z_index,] = 1
        # return aggregated_results, aggregated_nb_of_predictions
    ...
    def _internal_predict_3D_3Dconv_tiled(self, x: np.ndarray, step_size: float, do_mirroring: bool, mirror_axes: tuple,
        ...
        predicted_segmentation = np.zeros(data_shape[1:], dtype=np.uint8)
        data = np.ascontiguousarray(np.flip(data, axis=1))  ## rember filp mask

        if_stop = False
        for ii, x in enumerate(steps[0]):
            lb_x = x
            ub_x = x + patch_size[0]
            if ii > 1:
                if_stop = self.short_time_3D_prediction(aggregated_results[:, 0: lb_x - last_lb_x],
                                                        aggregated_nb_of_predictions[:, 0: lb_x - last_lb_x])
                if if_stop:
                    predicted_segmentation[last_lb_x: lb_x] = (
                            aggregated_results[:, 0: lb_x - last_lb_x] / aggregated_nb_of_predictions[:,
                                                                         0: lb_x - last_lb_x]).argmax(0).astype(
                        np.uint8)
                    break
            # if ii  == 0:
            #     aggregated_results = np.zeros([self.num_classes] + [ub_x] + list(data.shape[2:]), dtype=np.float32)
            #     aggregated_nb_of_predictions = np.zeros([self.num_classes] + [ub_x] + list(data.shape[2:]), dtype=np.float32)
            # else:
            #     expand_aggregated_results = np.zeros([self.num_classes] + [ub_x - aggregated_results.shape[1]] + list(data.shape[2:]), dtype=np.float32)
            #     expand_aggregated_nb_of_predictions = np.zeros([self.num_classes] + [ub_x - aggregated_results.shape[1]] + list(data.shape[2:]),
            #                                          dtype=np.float32)
            #     aggregated_results = np.concatenate([aggregated_results, expand_aggregated_results], axis=1)
            #     aggregated_nb_of_predictions = np.concatenate([aggregated_nb_of_predictions, expand_aggregated_nb_of_predictions], axis=1)
            if ii == 0:
                aggregated_results = np.zeros([self.num_classes] + [patch_size[0]] + list(data.shape[2:]), dtype=np.float32)
                aggregated_nb_of_predictions = np.zeros([self.num_classes] + [patch_size[0]] + list(data.shape[2:]), dtype=np.float32)
            else:
                predicted_segmentation[last_lb_x : lb_x] = (aggregated_results[:, 0 : lb_x - last_lb_x] / aggregated_nb_of_predictions[:, 0 : lb_x - last_lb_x]).argmax(0).astype(np.uint8)

                expand_aggregated_results = np.zeros([self.num_classes] + [ub_x - last_ub_x] + list(data.shape[2:]), dtype=np.float32)
                expand_aggregated_nb_of_predictions = np.zeros([self.num_classes] + [ub_x - last_ub_x] + list(data.shape[2:]),
                                                     dtype=np.float32)
                aggregated_results = np.concatenate([aggregated_results[:, lb_x - last_lb_x: ], expand_aggregated_results], axis=1)
                aggregated_nb_of_predictions = np.concatenate([aggregated_nb_of_predictions[:, lb_x - last_lb_x: ], expand_aggregated_nb_of_predictions], axis=1)
            ...
            for y in steps[1]:
                ...
                for z in steps[2]:
                    ...
                    if ii == 0:
                        ...
                        aggregated_results[:, : , lb_y:ub_y, lb_z:ub_z] += predicted_patch
                        aggregated_nb_of_predictions[:, : , lb_y:ub_y, lb_z:ub_z] += add_for_nb_of_preds


            if ii == len(steps[0]) - 1 :
                predicted_segmentation[lb_x : ub_x] = (aggregated_results / aggregated_nb_of_predictions).argmax(0).astype(np.uint8)
            last_lb_x = lb_x
            last_ub_x = ub_x
        #we process the aggregated_results if is has been stop early
        # if if_stop:
        #     predicted_segmentation = self.inwhole_3D_prediction_process(aggregated_results,
        ...
        predicted_segmentation = predicted_segmentation[slicer[1:]]
        predicted_segmentation = np.flip(predicted_segmentation, axis=0)
        predicted_segmentation = np.expand_dims(predicted_segmentation, axis=0).astype(np.uint8)
    ...
    def _internal_maybe_mirror_and_pred_3D(self, x: Union[np.ndarray, torch.tensor], mirror_axes: tuple,
        ...
        #     if m == 0:
        #         pred = self.inference_apply_nonlin(self(to_cuda(x, gpu_id=self.get_device()))).cpu()
        #         result_torch = 1 / num_results * pred
        #
        #     if m == 1 and (2 in mirror_axes):
        #         pred = self.inference_apply_nonlin(self(to_cuda(torch.flip(x, (4,)), gpu_id=self.get_device()))).cpu()
        #         result_torch += 1 / num_results * torch.flip(pred, (4,))
        #
        #     if m == 2 and (1 in mirror_axes):
        #         pred = self.inference_apply_nonlin(self(to_cuda(torch.flip(x, (3,)), gpu_id=self.get_device()))).cpu()
        #         result_torch += 1 / num_results * torch.flip(pred, (3,))
        #
        #     if m == 3 and (2 in mirror_axes) and (1 in mirror_axes):
        #         pred = self.inference_apply_nonlin(self(to_cuda(torch.flip(x, (4, 3)), gpu_id=self.get_device()))).cpu()
        #         result_torch += 1 / num_results * torch.flip(pred, (4, 3))
        #
        #     if m == 4 and (0 in mirror_axes):
        #         pred = self.inference_apply_nonlin(self(to_cuda(torch.flip(x, (2,)), gpu_id=self.get_device()))).cpu()
        #         result_torch += 1 / num_results * torch.flip(pred, (2,))
        #
        #     if m == 5 and (0 in mirror_axes) and (2 in mirror_axes):
        #         pred = self.inference_apply_nonlin(self(to_cuda(torch.flip(x, (4, 2)), gpu_id=self.get_device()))).cpu()
        #         result_torch += 1 / num_results * torch.flip(pred, (4, 2))
        #
        #     if m == 6 and (0 in mirror_axes) and (1 in mirror_axes):
        #         pred = self.inference_apply_nonlin(self(to_cuda(torch.flip(x, (3, 2)), gpu_id=self.get_device()))).cpu()
        #         result_torch += 1 / num_results * torch.flip(pred, (3, 2))
        #
        #     if m == 7 and (0 in mirror_axes) and (1 in mirror_axes) and (2 in mirror_axes):
        #         pred = self.inference_apply_nonlin(self(to_cuda(torch.flip(x, (4, 3, 2)), gpu_id=self.get_device()))).cpu()
